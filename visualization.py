...
def visualize_graph(graph: dataclasses.WeightedGraph,
    ...
    kinds = [graph_nx.nodes[k]['bias'] for k in graph_nx.nodes]

    # colours = [BOOK_COLOUR if kind > 0.5 else USER_COLOUR for kind in kinds]
    colours = ['rgb(' + str(round((kind - 0.5)*510)) + ', 0, 0)' if kind > 0.5 else
               'rgb(0, 0, ' + str(round((0.5 - kind) * 510)) + ')'for kind in kinds]
...
def visualize_graph_clusters(graph: dataclasses.WeightedGraph, clusters: list[set],
                             layout: str = 'spring_layout',
                             max_vertices: int = 5000,
                             output_file: str = '') -> None:
